...
User.findOne({
  ...
  if (res2) {
    res.status(200).send({ balance: res2.balance });
  }
});
});
...
app.post("/pay", (req, res) => {
  User.findOne({
    ...
    console.error("Error finding user:", saveError);
    ...
    if (res2) {
      User.findOneAndUpdate(
        ...
        const transaction = new Transaction({
          ...
          type: 'Topup',
          balanceBefore: Number(res2.balance),
          balanceAfter: Number(req.body.balance),
          status: 'Complete',
          date: getCurrentDate(),
          totalAmount: Number(req.body.balance) - Number(res2.balance)
        });
        transaction.save()
        .then((result) => {
