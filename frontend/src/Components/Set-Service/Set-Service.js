...
const SetService = () => {
  ...
  useEffect(() => {
    axios.get("http://16.171.177.188:8088/getAllServices").then((res) => {   
      if (res) {
  ...
  const handleAddService = () => {
    if (newService.trim() !== '' && newServiceRate.trim() !== '') {
      ...
      axios.post("http://16.171.177.188:8088/addService", data).then((res) => {
        if (res) {
  ...
  const handleRemoveService = (serviceToRemove) => {
    ...
    axios.post("http://16.171.177.188:8088/removeService", data).then((res) => {   
      if (res) {
  ...
  const handleSetRate = (service) => {
    ...
    axios.post("http://16.171.177.188:8088/setServiceRate", data).then((res) => {
      if (res) {
