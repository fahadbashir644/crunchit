...
const SetService = () => {
  ...
  useEffect(() => {
    axios.get("http://137.184.81.218/getAllServices").then((res) => {   
      if (res) {
  ...
  const handleAddService = () => {
    if (newService.trim() !== '' && newServiceRate.trim() !== '') {
      ...
      axios.post("http://137.184.81.218/addService", data).then((res) => {
        if (res) {
  ...
  const handleRemoveService = (serviceToRemove) => {
    ...
    axios.post("http://137.184.81.218/removeService", data).then((res) => {   
      if (res) {
  ...
  const handleSetRate = (service) => {
    ...
    axios.post("http://137.184.81.218/setServiceRate", data).then((res) => {
      if (res) {
